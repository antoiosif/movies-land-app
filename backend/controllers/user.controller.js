...
exports.insertUser = async (req, res) => {
  ...
  logger.info(`User with 'id=${result._id}' was inserted.`);
...
exports.getUser = async (req, res) => {
  ...
  logger.debug(`User with 'id=${userId}' was returned.`);
  res.status(httpStatusCodes.OK).json({status: true, data: result});
}
...
exports.getUsersFilteredSortedPaginated = async (req, res) => {
  ...
  logger.debug(`Filtered, sorted and paginated users were returned with page=${pagination.pageNumber} and size=${pagination.pageSize}.`);
  res.status(httpStatusCodes.OK).json({status: true, data: result});
}